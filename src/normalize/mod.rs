...
use kaspa_rpc_core::{RpcBlock, RpcHash, RpcTransaction, RpcTransactionId};
...
// ============================================================
// V1 fallback normalization (for nodes < v1.1.0)
// ============================================================

/// Create a ReorgEvent from removed block hashes
pub fn make_reorg_event(removed: &[RpcHash], sequence: &mut u64) -> proto::IngestorEvent {
    *sequence += 1;
    proto::IngestorEvent {
        sequence: *sequence,
        timestamp_ms: now_ms(),
        event: Some(proto::ingestor_event::Event::Reorg(proto::ReorgEvent {
            removed_block_hashes: removed.iter().map(|h| h.as_bytes().to_vec()).collect(),
        })),
    }
}

/// Normalize a full RpcBlock (from get_block) into a ChainBlockEvent
pub fn normalize_block_v1(
    block: &RpcBlock,
    accepted_tx_ids: &[RpcTransactionId],
    sequence: &mut u64,
) -> proto::IngestorEvent {
    let header = convert_rpc_header(&block.header);

    // Only include transactions that were accepted in this chain block
    let accepted_set: std::collections::HashSet<&RpcTransactionId> =
        accepted_tx_ids.iter().collect();

    let accepted_txs: Vec<proto::AcceptedTransaction> = block
        .transactions
        .iter()
        .filter(|tx| {
            tx.verbose_data
                .as_ref()
                .map(|v| accepted_set.contains(&v.transaction_id))
                .unwrap_or(false)
        })
    ...
    *sequence += 1;
    proto::IngestorEvent {
        sequence: *sequence,
        timestamp_ms: now_ms(),
        event: Some(proto::ingestor_event::Event::ChainBlock(
            proto::ChainBlockEvent {
                block_hash,
                header: Some(header),
...
fn convert_rpc_header(h: &kaspa_rpc_core::RpcHeader) -> proto::BlockHeader {
    let parents = h
        .parents_by_level
        .iter()
        .map(|level| proto::ParentLevel {
            parent_hashes: level.iter().map(|p| p.as_bytes().to_vec()).collect(),
        })
        .collect();

    proto::BlockHeader {
        hash: h.hash.as_bytes().to_vec(),
        version: h.version as u32,
        parents_by_level: parents,
        hash_merkle_root: h.hash_merkle_root.as_bytes().to_vec(),
        accepted_id_merkle_root: h.accepted_id_merkle_root.as_bytes().to_vec(),
        utxo_commitment: h.utxo_commitment.as_bytes().to_vec(),
        timestamp_ms: h.timestamp,
        bits: h.bits,
        nonce: h.nonce,
        daa_score: h.daa_score,
        blue_work: h.blue_work.to_be_bytes().to_vec(),
        blue_score: h.blue_score,
        pruning_point: h.pruning_point.as_bytes().to_vec(),
    }
}

fn convert_rpc_transaction(tx: &RpcTransaction) -> proto::AcceptedTransaction {
    let verbose = tx.verbose_data.as_ref();

    proto::AcceptedTransaction {
        transaction_id: verbose
            .map(|v| v.transaction_id.as_bytes().to_vec())
            .unwrap_or_default(),
        hash: verbose
            .map(|v| v.hash.as_bytes().to_vec())
            .unwrap_or_default(),
        ...
        inputs: tx
            ...
            .iter()
            .enumerate()
            .map(|(i, input)| proto::TransactionInput {
                index: i as u32,
                ...
                utxo_entry: None, // Not available in v1 get_block response
            })
            .collect(),
        outputs: tx
            .outputs
            ...
            .enumerate()
            .map(|(i, output)| {
                let out_verbose = output.verbose_data.as_ref();
                proto::TransactionOutput {
                    index: i as u32,
                    amount: output.value,
                    script_public_key: output.script_public_key.script().to_vec(),
                    script_public_key_version: output.script_public_key.version() as u32,
                    address: out_verbose
        ...
        subnetwork_id: {
            let bytes: &[u8; 20] = tx.subnetwork_id.as_ref();
            bytes.to_vec()
        },
        gas: tx.gas,
        payload: tx.payload.clone(),
        mass: tx.mass,
        block_time: verbose.map(|v| v.block_time).unwrap_or(0),
    }
}
